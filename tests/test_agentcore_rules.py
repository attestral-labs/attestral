...
AGENTCORE_RULES = {"ATL-070", "ATL-071", "ATL-072", "ATL-073", "ATL-074", "ATL-075"}
...
def test_rules_fire_on_the_weak_resources_only():
    ...
    # scoped oauth) never fire. The weak gateway trips two (LOG_ONLY + open JWT),
    # so six findings across six rules.
    assert len(findings) == 6
    assert {f.rule_id for f in findings} == AGENTCORE_RULES
    assert all("_weak" in f.component_id for f in findings)


def test_remote_target_unscoped_cred_derivation():
    model = build_model(FIXTURE)
    weak = next(c for c in model.by_type("aws_bedrockagentcore_gateway_target")
                if c.name == "crm_weak")
    hardened = next(c for c in model.by_type("aws_bedrockagentcore_gateway_target")
                    if c.name == "crm_hardened")
    # remote endpoint + no scoped provider (gateway_iam_role) -> stamped;
    # remote endpoint + oauth provider_arn -> not stamped.
    assert weak.attr("_remote_target_unscoped_cred") is True
    assert hardened.attr("_remote_target_unscoped_cred") is None
